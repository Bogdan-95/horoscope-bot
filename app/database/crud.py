...
# CRUD операции для базы данных (исправленная версия)
...
def create_user(user_id: int, username: Optional[str], first_name: Optional[str]) -> None:
    """Создает нового пользователя (упрощенная версия)"""
    try:
        conn = get_connection()
        cursor = conn.cursor()

        # Просто вставляем без проверки
        cursor.execute(
...
def get_subscription(user_id: int) -> Optional[Dict]:
    """Получает подписку пользователя (упрощенная версия)"""
    try:
        conn = get_connection()
        cursor = conn.cursor()
...
def get_subscribed_users_for_time(target_time: str) -> List[Dict]:
    """Получает пользователей с активной подпиской на указанное время (упрощенная)"""
    try:
        conn = get_connection()
        cursor = conn.cursor()
